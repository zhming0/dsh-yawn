...
/** The credential reference a Buildkite token is stored under by default. */
const DEFAULT_BUILDKITE_TOKEN_ENV = "BUILDKITE_API_TOKEN";
...
export function SandboxesSettings({
  ...
  const draftTokenRef =
    draft?.backend === "buildkite"
      ? draft.fields.tokenEnv?.trim() || DEFAULT_BUILDKITE_TOKEN_ENV
      : "";
  ...
  const write = async (
    ...
    setNotice(undefined);
    try {
      setView(await action(view.revision));
      return true;
    } catch (reason) {
      setError(describeError(reason));
  ...
  const saveProfile = async (event: FormEvent<HTMLFormElement>) => {
    ...
    // The token is written first: the profile is only usable once the host
    // can resolve it, and the credential document is host-side, never pushed
    // into a sandbox.
    ...
    if (saved) {
      ...
      const ref = draft.fields.tokenEnv?.trim() || DEFAULT_BUILDKITE_TOKEN_ENV;
      if (token !== "") {
        setToken("");
                    ...
                    <span>
                      {tokenInfo?.configured
                        ? tokenInfo.source === "env"
                          ? `Currently supplied by the environment variable ${draftTokenRef}. A token saved here is stored write-only and takes over when the variable is unset.`
