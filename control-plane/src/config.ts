...
export type ProfileConfig =
  ...
  | {
      ...
      tokenEnv?: string;
      /**
       * Credential reference the API token is stored under, typically entered
       * on the Sandboxes page. Defaults to a name derived from the profile.
       */
...
function resolveProfile(
  ...
  if (profile.backend === "buildkite") {
    return {
      ...
      tokenCredential:
        profile.tokenCredential ?? defaultBuildkiteTokenCredential(name),
    };
  }
  return {
    name,
