...
| `tokenCredential` | `buildkite`           | `DSH_YAWN_BUILDKITE_<PROFILE>_TOKEN` | Credential reference the UI-stored API token lives under          |
| `tokenEnv`        | `buildkite`           | `BUILDKITE_API_TOKEN`                | Environment variable holding the API token, as a fallback         |
...
1. the profile's `tokenCredential` reference, which **Settings → Sandboxes**
   writes through the write-only credential API; it is stored in the host
   credential document (`$DSH_HOME/.credentials.yaml`) and never reaches a
   sandbox. The default name is derived from the profile, so two Buildkite
   profiles keep two tokens; point both at one name to share a token.
2. `tokenEnv` in the process environment, the deployment-managed fallback.
