...
| `tokenEnv`        | `buildkite`           | `BUILDKITE_API_TOKEN`  | Credential reference holding the API token                        |
...
per Buildkite request from the credential reference named by `tokenEnv`: the
process environment first, then the host credential document
(`$DSH_HOME/.credentials.yaml`). **Settings → Sandboxes** writes that document
through the write-only credential API, so a token can be entered when the
profile is created — it stays host-side and never reaches a sandbox. A profile
whose token resolves nowhere does not stop the host: it is named once in the
log at boot, and its sessions fail at their first prompt with the setting to
fix. The pipeline shape, the registration token, and the limits are described
in
